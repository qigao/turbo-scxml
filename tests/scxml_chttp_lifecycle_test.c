...
static int allow_resolve(
    ...
    (void)uri;
    (void)uri_size;
    if (out_target == NULL) return SALTS_EINVAL;
...
spec("TurboSCXML CHTTP processor and binding lifecycle") {
    it("rejects partial zero and overflowing processor configurations") {
        ...
        check_equal(scxml_chttp_processor_init(NULL, &config), SALTS_EINVAL);
        check_equal(scxml_chttp_processor_init(&processor, NULL), SALTS_EINVAL);
        config.abi_version = 0u;
        check_equal(scxml_chttp_processor_init(&processor, &config), SALTS_EINVAL);
        ...
        check_equal(scxml_chttp_processor_init(&processor, &config), SALTS_ERANGE);
        config = test_processor_config();
    ...
    it("enforces processor state and fixed endpoint capacity") {
        ...
        check_equal(scxml_chttp_processor_init(&processor, &config),
                    SALTS_EALREADY);
        check_equal(scxml_chttp_processor_destroy(&processor), SALTS_EBUSY);
        ...
        check_equal(scxml_chttp_processor_start(&processor), SALTS_EALREADY);
        check_equal(scxml_chttp_binding_init(
                        &first, &processor, &first_config),
                    SALTS_OK);
        check_equal(scxml_chttp_binding_init(
        ...
        check_equal(scxml_chttp_binding_init(
                    ...
                    SALTS_ENOBUFS);
        check_equal(scxml_chttp_processor_stop(&processor, 1000u), SALTS_EBUSY);
        scxml_chttp_binding_event_io_adapter(&first)->close(
            scxml_chttp_binding_adapter_user(&first));
        check_equal(scxml_chttp_binding_destroy(&first), SALTS_OK);
    ...
    it("validates downstream adapters and delegates non-BasicHTTP requests") {
        ...
        check_false(adapter->is_quiescent(
            scxml_chttp_binding_adapter_user(&binding)));
        check_equal(scxml_chttp_binding_destroy(&binding), SALTS_EBUSY);
    ...
    it("derives optional delayed-send capabilities from the downstream") {
        ...
        check_equal(scxml_chttp_processor_start(&processor), SALTS_OK);
        check_equal(scxml_chttp_binding_init(
                        &binding, &processor, &binding_config), SALTS_OK);
        adapter = scxml_chttp_binding_event_io_adapter(&binding);
        check_not_null(adapter);
    ...
    it("recovers from worker creation failure and supports stop retry") {
        ...
        check_equal(scxml_chttp_processor_start(&processor), SALTS_ENOMEM);
        check_equal(scxml_chttp_processor_destroy(&processor), SALTS_OK);
    ...
    it("finishes cleanup after a terminal server error") {
        ...
        check_equal(scxml_chttp_processor_stop(&processor, 250u),
                    SALTS_ETIMEDOUT);
        check_equal(scxml_chttp_processor_stop(&processor, 1000u), SALTS_EIO);
        check_equal(scxml_chttp_processor_destroy(&processor), SALTS_OK);
    }
    ...
    it("binds a real session through the complete ownership protocol") {
        ...
        check_equal(scxml_chttp_binding_init(
                        ...
                        &unrelated_binding_config), SALTS_OK);
        ...
        check_equal(scxml_chttp_binding_activate(
                        &binding, &session, &unrelated_program), SALTS_EINVAL);
        check_equal(scxml_chttp_binding_activate(
                        &unrelated_binding, &session, &program), SALTS_EINVAL);
        check_equal(scxml_chttp_binding_activate(
                        &binding, &session, &program), SALTS_OK);
        check_equal(scxml_chttp_binding_activate(
                        &binding, &session, &program), SALTS_EALREADY);
        check_equal(scxml_chttp_binding_destroy(&binding), SALTS_EBUSY);
        ...
        check_equal(scxml_chttp_binding_destroy(&unrelated_binding), SALTS_OK);
        check_equal(scxml_chttp_processor_stop(&processor, 1000u), SALTS_OK);
        check_equal(scxml_chttp_processor_destroy(&processor), SALTS_OK);
