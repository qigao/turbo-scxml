...
static int w3c_chttp_resolve(
    ...
    if (probe == NULL || uri == NULL || out_target == NULL ||
        probe->access_uri_size == 0u ||
        uri_size != probe->access_uri_size ||
...
static scxml_adapter_status w3c_chttp_downstream_cancel(
    ...
    if (user == NULL || request == NULL || out_ticket == NULL ||
        ...
        return SCXML_ADAPTER_INVALID_CONTRACT;
    *out_ticket = (cflow_statechart_effect_ticket){
        w3c_chttp_downstream_discard,
        w3c_chttp_downstream_discard, user};
    *out_error = NULL;
    return SCXML_ADAPTER_ACCEPTED;
}
...
static bool run_w3c_chttp_fixture(const char *fixture_name) {
    ...
    if (scxml_chttp_binding_init(
        ...
        probe.descriptor.location_size >= sizeof(probe.access_uri))
        goto cleanup;
    memcpy(probe.access_uri, probe.descriptor.location,
           probe.descriptor.location_size);
    probe.access_uri[probe.descriptor.location_size] = '\0';
    probe.access_uri_size = probe.descriptor.location_size;
    ...
    session_config = (scxml_session_config){
        ...
        .event_io = scxml_chttp_binding_event_io_adapter(&probe.binding),
