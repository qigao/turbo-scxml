...
static bool decode_entities(
    ...
    while (cursor < input.size) {
        ...
        if (input.data[cursor] != '&') {
            if (!append_byte(output, capacity, &decoded_size,
                             input.data[cursor++]))
                return false;
            continue;
        }
        if (input.size - cursor >= 4u &&
            memcmp(input.data + cursor, "&lt;", 4u) == 0) {
            decoded = '<'; consumed = 4u;
        } else if (input.size - cursor >= 4u &&
                   memcmp(input.data + cursor, "&gt;", 4u) == 0) {
            decoded = '>'; consumed = 4u;
        } else if (input.size - cursor >= 5u &&
                   memcmp(input.data + cursor, "&amp;", 5u) == 0) {
            decoded = '&'; consumed = 5u;
        } else if (input.size - cursor >= 6u &&
                   memcmp(input.data + cursor, "&quot;", 6u) == 0) {
            decoded = '"'; consumed = 6u;
        } else if (input.size - cursor >= 6u &&
                   memcmp(input.data + cursor, "&apos;", 6u) == 0) {
            decoded = '\''; consumed = 6u;
        ...
        if (consumed != 0u) {
            if (!append_byte(output, capacity, &decoded_size, decoded))
                return false;
            cursor += consumed;
            continue;
        }
        if (input.size - cursor >= 4u && input.data[cursor + 1u] == '#') {
            const bool hexadecimal =
                cursor + 2u < input.size &&
                (input.data[cursor + 2u] == 'x' ||
                 input.data[cursor + 2u] == 'X');
...
static void measurement_destroy(vxml_measurement *measurement) {
    ...
    for (index = 0u; index < measurement->id_count; ++index)
        free(measurement->ids[index].data);
    free(measurement->ids);
...
static vxml_status append_id(
    ...
    if (attribute.impl != NULL) {
        ...
        id.data = (char *)malloc(retained_size);
        if (id.data == NULL) {
            return fail(
    ...
    if (measurement->id_count == measurement->id_capacity) {
        ...
        ids = (vxml_decoded_id *)realloc(measurement->ids, allocation_size);
        if (ids == NULL) {
            free(id.data);
            return fail(
                diagnostic, VXML_ALLOCATION_FAILED,
...
static vxml_status measure_document(
    ...
    if (salts_xml_node_type(root) != SALTS_XML_ELEMENT ||
        ...
        !view_equal(salts_xml_node_namespace_uri(root), VXML_NAMESPACE)) {
        return fail(
    ...
    if (version.impl == NULL ||
        (!view_equal(salts_xml_attribute_value(version), "2.0") &&
         !view_equal(salts_xml_attribute_value(version), "2.1"))) {
        return fail(
...
vxml_status vxml_compile(const void *bytes, size_t size,
    ...
    if (xml_status != SALTS_XML_OK) {
        vxml_status mapped = VXML_XML_ERROR;
        if (xml_status == SALTS_XML_LIMIT_EXCEEDED)
            mapped = VXML_LIMIT_EXCEEDED;
        else if (xml_status == SALTS_XML_ALLOCATION_FAILED)
            mapped = VXML_ALLOCATION_FAILED;
        return fail(
    ...
    impl = (vxml_program_impl *)calloc(1u, allocation_size);
    if (impl == NULL) {
        status = fail(
            diagnostic, VXML_ALLOCATION_FAILED,
            salts_xml_node_location(salts_xml_document_root(&document)),
