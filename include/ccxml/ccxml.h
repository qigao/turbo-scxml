...
/** Opaque single-owner condition compiled by a datamodel adapter. */
typedef struct ccxml_condition {
    void *impl;
} ccxml_condition;
...
/** Borrowed request fields valid only during one prepare callback. */
typedef struct ccxml_prepared_dialog_start_request {
    const char *dialog_id;
    size_t dialog_id_size;
    const char *connection_id;
    size_t connection_id_size;
} ccxml_prepared_dialog_start_request;
...
typedef struct ccxml_datamodel_adapter_v1 {
    ...
    /** Optional tail operations required by transition-condition programs. */
    scxml_adapter_status (*compile_condition)(
        void *user, const char *source, size_t source_size,
        ccxml_condition *out_condition, const char **out_error);
    /** Evaluate without mutation; event fields are borrowed for this call. */
    scxml_adapter_status (*evaluate_condition)(
...
typedef struct ccxml_telephony_adapter_v1 {
    ...
    scxml_adapter_status (*prepare_destroy_conference)(
        ...
        const char **out_error);
    /**
     * Optional tail operation, required by dialogstart programs.
     * ACCEPTED must publish a nonempty borrowed identifier as well as a valid
     * ticket. The identifier remains valid until that ticket is committed or
     * discarded; the core asks its datamodel adapter to copy the identifier
     * before resolving the provider ticket.
     */
    scxml_adapter_status (*prepare_dialog_start)(
        void *user,
    ...
    scxml_adapter_status (*prepare_dialog_prepare)(
        ...
        cflow_statechart_effect_ticket *out_ticket,
        const char **out_error);
    /** Optional tail operation, required by prepared dialogstart programs. */
    scxml_adapter_status (*prepare_prepared_dialog_start)(
        void *user,
