...
`TurboSCXML::SCXML`, `CHttp::Client`, and `CHttp::Server` from the standalone CHTTP package. The core remains transport-free.
...
an unknown endpoint, decoder/data validation failure, an invalid Event name,
or a full/closed mailbox returns a non-2xx response. A valid Event with no
matching transition is still admitted and returns 204; routing does not define
HTTP success.
...
send id, and encoded body. The one `Content-Type` header is constructed from
row-owned codec metadata only when the worker submits the request; it is not a
separate stored stride. No pointer from `scxml_send_request`, the resolver, or
the session is retained without an associated live binding reference.

The checked aggregate contains the copied advertised authority, base path,
route path, ingress decode text, aligned ingress form-entry views, aligned
endpoint rows, endpoint access-URI strides, aligned egress rows, aligned cancel
tickets, and every egress string/body stride. Conceptually:

```text
total_bytes = authority_bytes + base_path_bytes + route_path_bytes
            + ingress_text_bytes
            + align(form_entry_count * form_entry_view_bytes)
            + align(endpoint_capacity * endpoint_row_bytes)
            + endpoint_capacity * access_uri_stride
            + align(egress_capacity * egress_row_bytes)
            + align(egress_capacity * cancel_ticket_bytes)
            + egress_capacity * (
                  connection_uri_stride + authority_stride + target_stride
                + send_id_stride + encoded_body_stride)
```

`layout_take()` accounts for every alignment pad and checks every addition and
multiplication. A zero capacity, zero string bound, body bound larger than
CHTTP's configured request bound, or aggregate overflow fails initialization.

Slot states are:

```text
FREE -> RESERVED -> READY -> SUBMITTING -> SUBMITTED
          |           |          |             |
          |           |          |             +--callback--> COMPLETING
          |           |          +--immediate failure-------> COMPLETING
          |           |          +--ENOBUFS-----------------> READY
          |           +--cancel commit----------------------> FREE
          +--discard----------------------------------------> FREE
COMPLETING -> report completion/error outside lock -> FREE
```

READY may also carry a future due time. CANCEL_RESERVED is a transient ticket
state. Cancel admission claims a matching send id in RESERVED, READY,
SUBMITTING, SUBMITTED, or COMPLETING so a race cannot incorrectly fall through
to the downstream adapter. Commit generation-checks the row, frees READY,
marks SUBMITTING/SUBMITTED for worker-owned cancellation, and is a no-op if the
same generation is still RESERVED or already COMPLETING. A changed generation
is also a successful no-op. Immediate submit failure enters COMPLETING from
SUBMITTING. COMPLETING deliberately retains the binding/session reference
while completion notifications are reported outside the processor mutex.
...
through COMPLETING. A transport error or final non-2xx response calls
`scxml_session_report_adapter_error(...COMMUNICATION)` outside the processor
mutex while the binding reference still protects the session; only afterward
does it release the row. There are no redirects, retries, method changes, or
HTTPS downgrade. URI interpretation and authorization live solely in the
resolver.
