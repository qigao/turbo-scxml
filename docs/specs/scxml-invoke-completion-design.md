...
| 235 | ID `foo` 的完成匹配 `done.invoke.foo`，且 CMeta guard 直接要求 `_event.name == "done.invoke.foo"` | `namespace/test235.scxml` | `SCXMLInvoker::run()` 构造 `done.invoke.` + child invoke ID | 采用精确 envelope 名称；核心仍用 descriptor 的有限 compiled Event ID | descriptor + live row | `test235.scxml` + 同 event fail fallback + 完成统计 |
