...
**Tech Stack:** C11, TurboSCXML, TurboUtils CFlow/CMeta/Core/TinyTest, TurboUtils CHTTP/CNet, CMake Presets, W3C SCXML 1.0.
...
- `TURBOSCXML_ENABLE_CHTTP_EVENT_IO` defaults to `OFF`. When `ON`, resolve `TurboUtils::CHTTP` only from the already selected `TURBOUTILS_ROOT`; never add a second package root or fallback search.
- Feature-ON execution requires a CHTTP-enabled TurboUtils SDK installed at `$env{PKG_ROOT}/turboutils/{debug,release}` before configuring TurboSCXML.
...
Install the CHTTP-enabled TurboUtils SDK into the ordinary Release root, then
...
core still configures when `TurboUtils::CHTTP` is absent.
...
if(TURBOSCXML_ENABLE_CHTTP_EVENT_IO)
  if(NOT TARGET TurboUtils::CHTTP)
    message(FATAL_ERROR
      "TURBOSCXML_ENABLE_CHTTP_EVENT_IO requires TurboUtils::CHTTP from TURBOUTILS_ROOT")
  endif()
  ...
  target_link_libraries(turbo_scxml_chttp_event_io
    PUBLIC TurboSCXML::SCXML TurboUtils::CHTTP
...
The package config continues to find only the one selected TurboUtils package;
...
**Interfaces:**
- Consumes: public versioned configs, TurboUtils mutex/condition/thread/UUID APIs, downstream `scxml_event_io_adapter`.
...
session_config.event_io = scxml_chttp_event_io_adapter();
...
complete it as communication failure. The CHTTP callback changes
`SUBMITTED -> FREE`, records status/metrics, releases the binding reference,
and reports adapter communication failure only after releasing the processor
mutex.

Implement cancel as a transaction over `{slot,generation}`. Prepare finds a
matching live BasicHTTP send id and reserves a cancel ticket; commit removes a
READY row or calls `chttp_async_request_cancel()` for SUBMITTED on the worker
thread through a control flag. If no BasicHTTP row matches, delegate to the
downstream cancel callback. Advertise delayed/cancel only when both downstream
...
- Create: `tests/install_consumer/consumer.cpp`
- Modify: `CMakeLists.txt`
...
```powershell
cmd /c "call \"\"C:\Program Files\Microsoft Visual Studio\2022\Professional\Common7\Tools\VsDevCmd.bat\"\" -arch=x64 -host_arch=x64 >nul && cmake --fresh --preset win-release-user && cmake --build --preset win-release-user && ctest --preset win-release-user --output-on-failure && cmake --build --preset install-win-release-user"
```
...
```powershell
cmd /c "call \"\"C:\Program Files\Microsoft Visual Studio\2022\Professional\Common7\Tools\VsDevCmd.bat\"\" -arch=x64 -host_arch=x64 >nul && cmake --fresh --preset win-dev-user && cmake --build --preset win-dev-user && ctest --preset win-dev-user --output-on-failure && cmake --build --preset install-win-dev-user"
```
...
non-CHTTP TurboUtils SDK, build, run complete CTest, install, and run the core
...
Implemented on `feat/chttp-event-io`. Fresh completion gates passed on
2026-09-02:

- Release CHTTP build: 16/16 CTest tests, including all 13 promoted
  BasicHTTP W3C rows;
- Debug CHTTP build under MSVC AddressSanitizer: 16/16 CTest tests, with the
  toolchain ASan runtime added to `PATH`;
- Release and Debug CHTTP installs: core, resource C/C++, and Event I/O C/C++
  consumers configured, linked, and ran;
- Release feature-OFF build: 11/11 CTest tests, core-only install consumer,
  and exact transport-free `TurboSCXML::SCXML` link interface;
- negative component check: the core-only installed package rejects a
  `CHttpEventIO` component request.
