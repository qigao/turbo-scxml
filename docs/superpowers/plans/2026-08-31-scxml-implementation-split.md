...
**Tech Stack:** C11, CMake/Ninja, Salts CFlow/CMeta/QueryVM/XmlParser, TinyTest.
...
- Keep real Salts CFlow and CMeta types named `cflow_*` and `cmeta_*`.
...
- [x] **Step 2: Reconfigure, build, and run the install consumer**
  ...
  Configure `tests/install_consumer` with `TURBOSCXML_ROOT` and `SALTS_ROOT` set to the matching Release roots, build it with Ninja, then run `turboscxml_install_consumer.exe`.
